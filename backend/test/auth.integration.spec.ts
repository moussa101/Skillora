...
describe('Auth Integration', () => {
    ...
    describe('Full registration flow', () => {
        it('should register user, verify email, then allow login', async () => {
            // Step 1: Register
            const registerDto = { email: 'flow@example.com', password: 'Password123!', name: 'Flow User' };
            
            mockPrisma.user.findUnique.mockResolvedValueOnce(null); // No existing user
            const createdUser = {
                id: 1, email: registerDto.email, name: registerDto.name,
                password: '$2b$10$hashedpassword', provider: 'EMAIL',
                emailVerified: false, role: 'CANDIDATE', tier: 'GUEST',
                onboardingComplete: false, userType: 'STUDENT',
                analysesThisMonth: 0, image: null,
            };
            ...
            mockPrisma.verificationCode.findFirst.mockResolvedValue({
                id: 1, email: registerDto.email, code: '123456', used: false,
                expiresAt: new Date(Date.now() + 900000),
            });
            mockPrisma.verificationCode.update.mockResolvedValue({});
            mockPrisma.user.update.mockResolvedValue({ ...createdUser, emailVerified: true });

            const verifyResult = await authController.verifyEmail({ email: registerDto.email, code: '123456' } as any);
    ...
    describe('JWT token generation and validation', () => {
        it('should generate a valid JWT token on login', async () => {
            const user = {
                id: 1, email: 'token@example.com', role: 'CANDIDATE',
                tier: 'GUEST', name: 'Token User', image: null,
        ...
        it('should include correct user info in response', async () => {
            const user = {
                id: 2, email: 'pro@example.com', role: 'CANDIDATE',
                tier: 'PRO', name: 'Pro User', image: 'https://example.com/img.jpg',
                userType: 'PROFESSIONAL', emailVerified: true,
                onboardingComplete: true, analysesThisMonth: 3,
            };
    ...
    describe('Password reset flow', () => {
        it('should handle forgot → reset password flow', async () => {
            ...
            mockPrisma.user.findUnique.mockResolvedValueOnce({
                id: 1, email: 'reset@example.com', provider: 'EMAIL',
            });
            mockPrisma.verificationCode.deleteMany.mockResolvedValue({ count: 0 });
            mockPrisma.verificationCode.create.mockResolvedValue({});

            const forgotResult = await authController.forgotPassword({ email: 'reset@example.com' } as any);
            expect(forgotResult.success).toBe(true);

            // Reset password
            mockPrisma.verificationCode.findFirst.mockResolvedValue({
                id: 1, email: 'reset@example.com', code: '654321', used: false,
                expiresAt: new Date(Date.now() + 900000),
            });
            mockPrisma.verificationCode.update.mockResolvedValue({});
            ...
            const resetResult = await authController.resetPassword({
                email: 'reset@example.com', code: '654321', newPassword: 'NewPassword123!',
            } as any);
    ...
    describe('OAuth callback integration', () => {
        it('should generate auth response for GitHub OAuth user', async () => {
            const githubUser = {
                id: 1, email: 'github@example.com', name: 'GitHub User',
                image: 'https://github.com/avatar.jpg', role: 'CANDIDATE',
                tier: 'GUEST', userType: 'STUDENT', emailVerified: true,
                onboardingComplete: true, analysesThisMonth: 0,
            };
    ...
    describe('Tier-based usage limits', () => {
        ...
        it('should allow unlimited for PRO tier', async () => {
            mockPrisma.user.findUnique.mockResolvedValue({
                id: 1, tier: 'PRO', analysesThisMonth: 100,
                analysesResetDate: new Date(),
            });
            mockPrisma.user.update.mockResolvedValue({});
