...
export class SubscriptionService {
   ...
   */
  async getStats() {
    const [pending, approved, rejected, expired, totalRevenue] =
      await Promise.all([
  ...
  async approveSubscription(
    ...
    this.logger.log(
      `Subscription #${subscriptionId} approved for user #${subscription.userId} (${subscription.plan}) until ${endDate.toISOString()}`,
    );
    return updatedSub;
  }
  ...
  async handleExpiredSubscriptions() {
    ...
    this.logger.log(
      `Found ${expiredSubs.length} expired subscription(s), downgrading...`,
    );
    ...
    for (const sub of expiredSubs) {
      ...
      this.logger.log(
        `User #${sub.userId} (${sub.user.email}) downgraded to GUEST — subscription expired`,
      );
    }
  }
}
